...
2. **A React + TypeScript frontend** with a dashboard, a live-streaming
   ...
   Server-Sent Events), full report history, saved companies, and account
   settings — built with shadcn-style components on Tailwind v4, Framer
   Motion for the interface's only deliberately loud element (the verdict
   stamp), and Clerk for auth.
...
├── backend_python/           # Original Python FastAPI backup
...
backend is running. Summary of the REST surface:
...
- **Dev-mode auto-user is a deliberate, hard-coded single user**, not a
  ...
  `APP_ENV=production`.
- **Backend Stack Rewrite (Python to Node.js/TypeScript)**: The initial backend prototype was built using Python and FastAPI. However, to align with the take-home assignment's explicit tech stack guidelines (preferring Node.js/TypeScript for production-grade builds), the backend was fully rewritten in Express and LangGraph.js. This choice maintains consistency in language ecosystem (TypeScript on both frontend and backend) and leverages Prisma's robust, type-safe database queries. The original Python backend is preserved in `backend_python/` for reference.
...
To provide insight into the development process, here are the transcript logs of the collaboration sessions with the AI Coding Assistant. Rather than building the product from scratch, the AI was leveraged for reference, diagnostic database/SMTP testing, routing fixes, stream endpoint email bug resolution, and responsive HTML email formatting.

<details>
<summary><b>Session 1: Database Setup & Server Orchestration</b></summary>

* **User Query**: `run everything and provide a front-end link`
* **Context**: Initial environment checks, Prisma ORM sync, Node/Vite development server launching.
* **AI Actions & Output**:
  1. Ran Prisma sync `npx prisma db push` to align the database tables with Neon Postgres.
  2. Bypassed restricted PowerShell execution policies under Windows using `cmd /c` overrides.
  3. Spun up the Express backend server on port `8000`.
  4. Spun up the Vite React frontend server on port `5173`.
  5. Verified connection health checking on `http://localhost:8000/api/v1/health` (returned `{"status":"ok"}`).
  6. Shared local links for testing.
</details>

<details>
<summary><b>Session 2: Dashboard Gating & Route Redirects</b></summary>

* **User Query**: `after login it initially starts with dashboard only no home page to appear`
* **Context**: Navigating back to the root route (`/`) while logged in was showing the public landing page instead of the dashboard. Also, the Clerk login component redirected back to `/` by default.
* **AI Actions & Output**:
  1. Wrapped the root route `/` in `PublicOnlyRoute` in `App.tsx` so that signed-in users are automatically redirected to `/dashboard`.
  2. Configured the `forceRedirectUrl="/dashboard"` property on the Clerk `<SignIn>` and `<SignUp>` components to override Clerk's default landing page redirection behavior.
</details>

<details>
<summary><b>Session 3: Debugging Missing Email Notifications</b></summary>

* **User Query**: `searched for claude in research, email is turned on but still didn't receive any email`
* **Context**: Database inspection showed 10 completed reports, but no notification email was received.
* **AI Actions & Output**:
  1. Wrote and ran a database/SMTP diagnostics script to test nodemailer connection directly. Verified that nodemailer successfully connected and sent a test email.
  2. Inspected `backend/src/routers/stream.ts` and realized that the dashboard uses the live **streaming endpoint** (`/api/v1/analyze/stream`) to update the UI in real-time, which completely bypassed the email-sending logic present only in the background job queue runner.
  3. Extracted the email notification SMTP block into a shared helper function `sendReportEmail` in `reportService.ts`.
  4. Imported and integrated `sendReportEmail` inside the stream completion logic in `stream.ts`.
</details>

<details>
<summary><b>Session 4: CSS Email Styling Touch-ups</b></summary>

* **User Query**: `got the mail but the template is too normal, make it more friendly using CSS`
* **Context**: NodeMailer default text templates were plain text and lacked branding.
* **AI Actions & Output**:
  1. Designed a responsive, table-based dark-theme HTML email template.
  2. Styled custom dynamic **Verdict Outcome stamps** (Green for `INVEST`, Red for `PASS`) following the product design tokens.
  3. Added inline-CSS horizontal progress bar indicators for the **Investment Score** and **Confidence Score**.
  4. Injected a stylized CTA button linking directly to the full analysis report on localhost.
</details>

<details>
<summary><b>Session 5: Production Cleanup</b></summary>

* **User Query**: `present it as a product, remove mock test and all testing to make it a final product`
* **Context**: Pre-presentation cleanup.
* **AI Actions & Output**:
  1. Verified environment values, ensuring `MOCK_LLM=false` was active.
  2. Deleted temporary diagnostic test scripts (`test_tools_debug.js` etc.) from the workspace.
  3. Documented steps to build and preview the app in production mode.
</details>
